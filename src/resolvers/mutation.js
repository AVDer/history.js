...
exports.newLeader = async (parent, args, {models}) => {
  ...
  return await models.Leader.create(
      {nameLatin: args.name, nameOriginal: args.name});
};
...
exports.signUp = async (parent, {username, email, password}, {models}) => {
  ...
  try {
    const user = await models.User.create({username, email, password: hashed});
...
exports.signIn = async (parent, {username, email, password}, {models}) => {
  if (email) {
    // normalize email address
    email = email.trim().toLowerCase();
  }
